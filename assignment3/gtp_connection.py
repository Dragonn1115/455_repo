...
class GtpConnection:
    def __init__(self, go_engine, board, debug_mode=False):
        ...
        for i in range(len(weights)):
            ...
            try:
                ...
                pass
        self.board.weights = self.weights
    ...
    def set_policy(self,args):
        policy_type = ["random","pattern"]
        assert args[0] in policy_type
        self.policy = args[0]
        self.board.policy = args[0]
        self.respond("")
        return

    def set_selection(self,args):
        selection_type = ["rr","ucb"]
        assert args[0] in selection_type
        self.selection = args[0]
        self.board.selection = args[0]
        self.go_engine.selection = args[0]
        self.respond("")
        return

    def policy_move(self, args):
                
        moves =self.board.legalMoves()
        prob = []
        if self.policy == "random":
            final = [round(1/len(moves),3)]*len(moves)

        elif self.policy == "pattern":
            for i in range(len(moves)):
                code_list = self.board.generate_neighbors_list(moves[i])
                reversed(code_list)
                x = 0
            ...
            final = [round(prob[i]/sum(prob),3) for i in range(len(prob))]
        ...
        for i in range(len(moves)):
            ...
            moves[i] = move_as_string.lower()

        str_final = [str(final[i]) for i in range(len(final))]

        temp = dict()
        for i in range(len(moves)):
            temp[moves[i]] = str_final[i]

        result = {k: v for k, v in sorted(temp.items(), key=lambda item: item[0])}
        response = " ".join(result.keys()) + " " + " ".join(result.values())
