...
class MCTS:
    ...
    def search(self, board: GoBoard, color: GO_COLOR) -> None:
        ...
        while not node.is_leaf():
            ...
            move, next_node = self.weight_selection(board,node)
    ...
    def weight_selection(self,board,node):
        ...
        for move, child in node.children.items():
            cboard = board.copy
            weight = cboard.modify_weight(child.move)
            if weight*coe > coe*_weight:
                _weight = weight
                _node = child

        return _node.move, _node
